...
int CPP_Errors::create_files() {
  ...
  */
  ...
  fprintf(out_h, _("  static unsigned int ErrorCode_To_Name( int code, char "
                   "*name, int len );\n"));
  fprintf(
      out_h,
      _("  static unsigned int ErrorCode_To_Level( int code, int *level );\n"));
  fprintf(out_h, _("  static unsigned int ErrorCode_To_Response( int code, int "
                   "*response );\n"));
  fprintf(out_h, _("  static unsigned int ErrorCode_To_Message( int code, char "
                   "*message, int len );\n"));
...
int CPP_Errors::parse_errors() {
  ...
  for (cnt1 = 0; cnt1 < errdef.NumberOfErrors(); cnt1++) {
    ...
    fprintf(out_h, _("const int %s_%-20s         = 0x%08X;\n"), ERROR_PREFIX,
            name.c_str(), code);
    fprintf(out_cpp, _("   { %s_%-20s, \"%s\", %d, %d, %s }"), ERROR_PREFIX,
