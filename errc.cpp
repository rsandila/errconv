...
#define _(str) str
#endif
C_Errors::C_Errors( const std::string & out_base, const Error_Definitions & err ) : base(out_base), errdef(err)
{
  installed=Init();
}
...
int C_Errors::parse_errors()
{
  ...
  fprintf( out_c, _("   { %d, \"%s\", %d, %d, %s }"), code, name.c_str(), level, response, message.c_str() );
