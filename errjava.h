...
/*! \brief This class will take the data as provided by the Error_Defintions
   class and write it to Java classes \pre The Error_Definitions class must be
   properly intialized. The Error_Definitions class passed to it must be valid
   for the whole life of this class. \post Three new files will be created
   [out_base]ImpLevel.java, [out_base]Response.java and [out_base]Error.java
   with classes with the same names as the files without the .java that will
   handle error processing and reporting.
